    ...
    class StudentExport
    {
        public void exportDataToPDF(DataGridView DGV, string filename)
        {
            ...
            foreach (DataGridViewRow row in DGV.Rows)
            {
                ...
                DateTime dt = (DateTime)row.Cells[3].Value;
                PdfPCell Bdate = new PdfPCell(new Phrase(dt.ToString("dd/MM/yyyy"), times)) { VerticalAlignment = Element.ALIGN_MIDDLE, HorizontalAlignment = Element.ALIGN_CENTER };
                pdfTable.AddCell(Bdate);
                PdfPCell gender = new PdfPCell(new Phrase(row.Cells[4].Value.ToString().Trim(), times)) { VerticalAlignment = Element.ALIGN_MIDDLE, HorizontalAlignment = Element.ALIGN_CENTER };
                pdfTable.AddCell(gender);
                PdfPCell phone = new PdfPCell(new Phrase(row.Cells[5].Value.ToString().Trim(), times)) { VerticalAlignment = Element.ALIGN_MIDDLE, HorizontalAlignment = Element.ALIGN_CENTER };
                pdfTable.AddCell(phone);
                PdfPCell address = new PdfPCell(new Phrase(row.Cells[6].Value.ToString().Trim(), times)) { VerticalAlignment = Element.ALIGN_MIDDLE, HorizontalAlignment = Element.ALIGN_CENTER };
        ...
        public void exportDataToExcel(System.Data.DataTable dt, string sheetName, string title, string filename)
        {
...

            {
                ...
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    ...
                    //    oSheet = oExcel.Workbooks.get_Item(1).Worksheets.get_Item(1) as Excel.Worksheet;
                    //    Excel.Range co = oSheet.get_Range("H" + (r + 4), "H" + (r + 4));
                    //    oSheet.Paste(co, false);
                    //    /*arr[r, c] = finalPic;*/
                    //    continue;
                    //}
                    arr[r, c] = dr[c];
        ...
        public void exportDataToWord(DataGridView DGV, string filename)
        {
            if (DGV.Rows.Count != 0)
            {
                ...
                Word.Paragraph tittle;
                tittle = doc.Paragraphs.Add();
                tittle.Range.Text = "Student List";
                tittle.Range.Font.Size = 26;
                tittle.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;
                tittle.Range.Bold = 1;
                tittle.Range.Font.Name = "Comic Sans MS";




                Range rng = doc.Range(12, 12);

                Table wdTable = doc.Tables.Add(rng, DGV.Rows.Count, DGV.Columns.Count);//make a new table based on our data grid view
                ...
                foreach (DataGridViewColumn column in DGV.Columns)
                {
                    ...
                    wdTable.Cell(1, next + 1).Range.Borders.OutsideLineStyle = WdLineStyle.wdLineStyleDouble;
                    wdTable.Cell(1, next + 1).Range.Borders.OutsideColor = WdColor.wdColorLightOrange;
                ...
                for (int r = 1; r <= RowCount-1; r++)
                {
                    ...
                    for (int c = 0; c <= ColumnCount - 1; c++)
                    {
                        ...
                        if (c == 3)
                        {
                            DateTime dt = (DateTime)DGV.Rows[r - 1].Cells[c].Value;
